    ...
    <DashboardLayout>
        <div class="absolute main-content ml-16 mt-16 overflow-y-auto">
            <div class="my-4 sm:my-12 mx-8 2xl:mx-auto 2xl:w-9/12">
                ...
                <form @submit.prevent="form.post(route('devis.store'))">
                    <div class="flex flex-wrap justify-between">
                        <div>
                            <h3 class="text-2xl font-bold text-left text-black">Nouveau devis</h3>
                        </div>
                        ...
                        <div class="grid gap-y-6 gap-x-4 grid-cols-1 md:grid-cols-3 col-span-12">
                            <div class="relative w-full text-left">
                                ...
                                <div class="flex flex-col mt-1">
                                    <div class="relative">
                                        <input v-model="form.estimateDate"
                            ...
                            <div class="relative w-full text-left">
                                ...
                                <div class="flex flex-col mt-1">
                                    <div class="relative">
                                        <input v-model="form.estimateDueDate"
                                               class="font-base py-2 outline-none focus:ring-primary-400 focus:outline-none focus:border-primary-400 block w-full sm:text-sm border-gray-200 rounded-md text-black form-control input"
                                               tabindex="0" type="date">
                                    </div>
                                </div>
...
export default {
    ...
    data() {
        return {
            form : useForm({
                ...
                estimateDueDate: this.getEstimateDueDate(),
                estimateId: this.generateIdEstimate(),
                discountRate: 0,
                sousTotal: 0,
                total: 0,
                paymentPenalityDays: 30,
                paymentPenalityRate: 6.37,
                notes: '',
                items: [
                    {id: 0, name: 'Intitulé de la prestation', description: 'Description de la prestation', quantity: 1, price: 0},
                ],
            }),
        }
    },
    methods: {
        getEstimateDueDate: function() {
            const date = new Date();
            date.setMonth(date.getMonth() + 1);
            return date.toISOString().slice(0,10);
        },
        generateIdEstimate: function() {
            const date = new Date();
            return 'D-' + date.getFullYear() + ("0" + (date.getMonth() + 1)).slice(-2) + '-' + ("00" + this.countEstimate).slice(-3);
        },
        addNewItem: function () {
    ...
    computed: {
        ...
        discountTotal: function () {
            return this.subTotal * (this.form.discountRate / 100);
        },
        grandTotal: function () {
